...
| 层 | 位置 | 形态 | 数量（最近一次全量） |
|---|---|---|---|
| 单元测试 | joy-rs 各 crate 内 | 进程内、离线、确定性 | 255 通过 / 15 套件 / 0 失败 |
...
| 确定性 eval | evals/deterministic/*.jsonl | scripted 模型驱动同一 run_turn | 30/30 通过，gate exit 0 |
...
| crate | 用例 | 覆盖要点 |
|---|---|---|
| joyczl-state | 19 | SQL 契约、FTS5 检索（含 CJK 与乱码输入）、日历幂等、记忆后端 conformance、事实类别的归一化、提炼失败的退避 |
| joyczl-tools | 39 | 工具行为与输出文案（落点如实）、create_event 幂等、SKILL.md 校验、参数 schema 校验、执行的三道闸门与批准（含沙箱**真的**拦住了允许目录之外的写入）、超长输出落盘、子代理工具表里没有它自己 |
| joyczl-provider | 24 | 两种 wire format 转换、SSE 解析、429 退避重试（真 HTTP）、token 估算、本地推理（不需要 key 的 provider） |
| joyczl-loop | 16 | 护栏双出口、工具往返、流式增量顺序、打断收兵 |
...
| joyczl-app-server | 25 | 协议分发、interrupt、config/write 持久化、model/list、trace/usage 落盘、批准等待表（含「问了才跑」这条因果）与子代理 |
...
| joy eval（确定性 eval，release gate） | **30/30，exit 0**（要沙箱的那几条在没有可用沙箱的机器上如实计为跳过） |
