...
双护栏管的是「什么时候停」，护栏管的是「别把预算烧在原地打转」。没有它，模型
把同一个调用重复到 `max_iterations` 用完，每一轮都是真金白银。

两条判据（`guard::StallGuard`，纯函数、可单测）：

| 判据 | 触发条件 | 为什么需要 |
|---|---|---|
| 连续相同 | 同一 `(工具, 参数, 结果)` 连续到第 **3** 次 | 阈值取 3 而不是 2：一次重复可能是无害的复核 |
| 交替循环 | 最近 `2p` 个调用两两相同，`p ∈ 2..=4` | `A,B,A,B…` 每次都把「连续计数」**重置**，只做连续检测的话模型可以一路跑到预算用完 |

第三条细节：周期内全是同一个调用的情形要跳过（那是「重复」不是「交替」），
否则两遍同样的调用会被误报成周期 2。

命中之后做两件事，**都只作用于喂回模型的文本**：

1. 追加一段话（`reminder`）：说清「同一调用已经出现 N 次/你在按 p 步周期重复」，
   并给出下一步（换一条路，或把卡住的地方告诉用户）。
2. **换引用桩**（`stub`）：第 2 次以上重复且结果 ≥512 字符时，把重复的长正文换成
   「[name 第 N 次相同调用：结果与上一次完全相同（参数：…）]」。短结果不换 ——
   桩本身也有几十字符，省不下来还看不全。

`LoopResult.tool_calls` 里留的**永远是真结果**（trace 与 `ToolCompleted` 通知该看到
真的），只有 `ContentBlock::ToolResult` 的 `content` 会被改写。命中次数与最后那句
话进 `LoopResult.guard` → `TurnMeta.guard_hits` / `guard_note`，于是「这一轮卡过」
在 trace、驾驶舱与 REPL（打一行小字）里都看得见。
