...
历史**：历史会被滑窗裁、会被摘要压缩，而清单恰恰是「压缩之后最该还在的东西」。
注入的位置在 `build_system` 里排得也靠前 —— 排在记忆与方法之前，因为它写的是
**这一趟正在做的事**。

三条边界都是抄来的：整表替换（逐项编辑会让「谁删了第 3 项」没有答案）、同时只
允许一项 `in_progress`（deepseek-harness 把这做成必填的部署选项，我们直接取
false 那一支）、硬上限（hermes 的 20 项 / 4000 字符）。校验不过时**一个字都不改**：
半写进去的清单比没写更坏。

子代理的工具表里把它也去掉了（除了 `delegate_task`）：子代理写清单会把父轮的
计划表覆盖掉，而它本来也不该替父轮规划。

### MCP 工具的两条纪律（都在 `joyczl-mcp`）

* **同名不覆盖**：`ToolRegistry::register` 遇到已存在的名字会**拒绝**后到的那个并
  在 stderr 说明。内建工具、MCP 服务器、技能都可能撞名，而「悄悄换掉一个工具的
  实现」比少一个工具危险得多 —— 一个叫 `run_command` 的 MCP 工具绝不能顶掉真正的
  `run_command`。
* **名字塌了要改名并说出来**：`{server}_{tool}` 会塌（服务器 `a` 的工具 `b_c` 与
  服务器 `a_b` 的工具 `c` 都变成 `a_b_c`），重名的加 `_2`、`_3`…… 并且**打印一行**
...
60 秒。开着的时候调用**直接返回一句说明**，不再去敲门 —— 否则一台挂掉的服务器会让
每一轮都白等一个超时，模型还会一遍遍重试。冷到点后**半开**：放一次探针，成功就
清零（这是我们唯一能拿到的「它缓过来了」的证据），失败就重新开。
...
一条 `find /` 的输出能塞爆上下文，所以输出超过 8000 字符就截断。但**丢掉的东西
是没了** —— 所以先落一份完整原文到 `<home>/spill/<日期>/<时间戳>-<pid>.txt`，
再把「完整输出在 spill/…」告诉模型：

```
退出码 0
<前 8000 字符>
…（输出共 129043 字符，已截断到前 8000；完整输出在 spill/20260921/093015.123-4242.txt）
```

**顺序不能反**：先落盘、后截断。第一版写成了先截断再落盘，于是存下去的是截断后
那份 —— 等于白存（被测试当场抓住，那个测试断言的就是「落盘的字符数 > 8000」）。

落盘失败（或没配目录）**不影响返回值**：退回「截断 + 诚实标注」。磁盘写不进去
不该让命令本身的输出也拿不到。清理只按时间（7 天，启动时扫一遍），不做容量配额
—— 见 `docs/limitations.md`。
