...
|---|---|---|---|
| Unit tests | inside each joy-rs crate | in-process, offline, deterministic | 159 passed / 15 suites / 0 failed |
...
| Deterministic evals | evals/deterministic/*.jsonl | scripted model driving the same run_turn | 13/13 pass, gate exit 0 |
...
| joyczl-tools | 12 | tool behavior and output wording (honest destinations), create_event idempotence, SKILL.md validation, parameter errors as text |
| joyczl-provider | 13 | both wire formats, SSE parsing, 429 rate limiting, model metadata |
...
| joyczl-mcp | 23 | transport frames, handshake, tool registration, full OAuth flow against a local fake authorization server, token storage 0600 |
| joyczl-memory | 16 | gate fail-open, consolidation, Skills triggering/rescan, MEMORY.md mirror |
...
## Test report (full run, 2026-09-20)

| Suite | Result |
|---|---|
| cargo test --workspace (15 binary suites) | **159 passed / 0 failed** |
...
| joy eval (deterministic evals, release gate) | **13/13, exit 0** |
...
the model), consolidation counts, interrupt timing, config/write hot reload,
and `prereq`-based skipping. Every assertion is offline and reproducible.
